...
class Point(object):
    ...
    def move_to(self, x, y):
        self.total_traveled += self.get_distance_from(Point(x,y))
        self.x = x
        self.y = y
        self.moves_made += 1

    def move_by(self, dx, dy):
        self.total_traveled += self.get_distance_from(Point(self.x+dx, self.y+dy))
        self.x += dx
        self.y += dy
        self.moves_made += 1
    ...
    def get_distance_from_start(self):
        return(math.sqrt((self.initial_x - self.x)**2 + (self.initial_y - self.y)**2))
